...
pl_berri_bikes = pl_fixed_df.select(["Date", "Berri 1"])
...
# NB: pandas counts weekdays from 0-6 and polars from 1-7
...
pl_weekday_counts = pl_berri_bikes.group_by("weekday").agg(
    pl.col("Berri 1").sum().alias("Berri 1")
)
...
# No index in Polars :-) But we can rename the column values
weekdays = {
    1: "Monday",
    2: "Tuesday",
    3: "Wednesday",
...
pl_weekday_counts = pl_weekday_counts.with_columns(
    pl.col("weekday").map_elements(lambda s: weekdays[s]).alias("weekday_renamed")
)
